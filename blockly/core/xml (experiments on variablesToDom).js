...
Blockly.Xml.workspaceToDom = function(workspace, opt_noId) {
  ...
  var variables = workspace.getAllVariables(workspace);
  if (variables.length) {
...
Blockly.Xml.domToVariables = function(xmlVariables, workspace) {
  for (var i = 0, xmlChild; xmlChild = xmlVariables.children[i]; i++) {
    ...
    var scopeID = xmlChild.textContent.substring(8,28);
    var name = xmlChild.textContent.substring(31,xmlChild.textContent.length);

    console.log(workspace.getAllBlocks());
    var scope = workspace.getAllBlocks().filter(function(b) { return b.id == scopeID});
    var group = "state";
    ...
    Blockly.Solidity.createVariable(workspace, group, type, name, scope, id);

    Blockly.Solidity.updateWorkspaceNameFields(workspace);
    Blockly.Solidity.updateWorkspaceStateTypes(workspace);
    Blockly.Solidity.updateWorkspaceParameterTypes(workspace);
